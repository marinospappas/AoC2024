...
class AStar[T](g: Graph[T]) {

    val log: Logger = LoggerFactory.getLogger("AStar")

    Graph.aStarAlgorithm = true // ensure A* is used
            ...
            boundary:
                ...
                for connectedNode <- g.getConnected(curNode).filter( _._1 != start) do {
                    ...
                    if allPaths || !visited.contains(nextPathNode) then {
                        ...
                        val newTotalDistance = totalDistance + g.heuristic(connectedNode._1)
                        if totalDistance < distFromStart.getOrElse(connectedNode._1, Int.MaxValue) then {
                            ...
                            priorityQueue.add(PathNode(connectedNode._1, newTotalDistance))
...
case class PathNodeAStar[T](id: T | Null = null, var estCostToEnd: Int = 0) extends Comparable[PathNodeAStar[T]] {
    override def compareTo(other: PathNodeAStar[T]): Int = estCostToEnd.compareTo(other.estCostToEnd)
}